...
import { useSummaryByCategoryController } from "./useSummaryByCategoryController";
...
const SummaryByCategory = ({type}) => {
    const { 
        ...
        isLoadingCategory
    } = useSummaryByCategoryController({type});

    const options = {
        labels: totalByCategories?.map(item => item?.name),
        legend: {
        ...
        colors: totalByCategories?.map(item => item?.color),
        dataLabels: {
            enabled: true,
    ...
    const series = totalByCategories?.map(item => item?.total);
    ...
    return ( 
        <div className="bg-white dark:bg-white/[3%] p-6 rounded-xl min-h-[470px] flex flex-col">
            ...
            {(isLoadingTransaction && isLoadingCategory) && (
                <div className="h-full flex flex-1 items-center justify-center">
                    <Spinner className="w-10 h-10 dark:text-white/[10%]" />
                </div>
            )}

            {(!hasTransactions && !isLoadingTransaction && !isLoadingCategory) && (
            ...
            {(hasTransactions && !isLoadingTransaction && !isLoadingCategory) && (
                <>
                    <div className="w-96 h-96 m-auto relative flex justify-center items-center text-center">
                        ...
                        <div>
                            <span className="text-gray-900 dark:text-gray-50 font-semibold text-base">{formatCurrency(chartTotal)}</span>
                            <p className="text-sm text-gray-700 dark:text-gray-500 font-medium">Total { format(selectedDate, "MMMM", { locale: ptBR }) }</p>
                        </div>
                    </div>
